...
def decrypt_scramble_block(block):
    for _ in range(40):
...
def mix_in_stage(msg):
    initial_state = xor(Y_bytes, Z_bytes)
    mix_in = scramble_block(msg)
    mix_in = rotate_right(mix_in, 1+11)
    mix_in = xor(mix_in, X_bytes)
    mix_in = rotate_left(mix_in, 1+6)
...
m=b'0'*32
m0=cryptohash(m)
mix_in2=mix_in_stage(b'1'*32)
m1first=decrypt_scramble_block(xor(bytes.fromhex(m0),mix_in2))
m1=m1first+b'1'*32

r=pwn.connect('socket.cryptohack.org',13405)
line=json_recv()
print(line)
line=json_recv()
print(line)
request={"m1":m.hex(),"m2":m1.hex()}
json_send(request)
line=json_recv()
print(line)